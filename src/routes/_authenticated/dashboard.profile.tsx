...
function ProfilePage() {
  ...
  const [reqRole, setReqRole] = useState<AppRole>(primaryRole === "student" ? "instructor" : "admin");
  ...
  const runAccessChecks = useCallback(async () => {
    ...
    {
      const { error } = await supabase.from("enrollments").select("id", { head: true, count: "exact" }).eq("student_id", user.id);
    ...
    // 4. Read all user_roles (admin only via RLS)
    {
      ...
      results.push({
        ...
        pass: !error && (data?.length ?? 0) >= 0 ? (isAdmin ? (data?.length ?? 0) > 0 || true : false) : false,
    ...
    // 5. Read academy_settings sensitive cols (admin only)
    {
      ...
      results.push({ label: "Read sensitive academy settings (admin)", pass: !error, detail: error?.message });
    }
    // 6. Read all capstones (admin/instructor)
    {
      const { error } = await supabase.from("capstone_submissions").select("id", { head: true, count: "exact" }).neq("student_id", user.id);
      results.push({ label: "Review others' capstones (instructor/admin)", pass: !error, detail: error?.message });
    }
  ...
  const roleOptions: AppRole[] = (["student", "instructor", "admin"] as AppRole[]).filter((r) => r !== primaryRole);
  const allowedPerms = PERMISSIONS[primaryRole];

  return (
    <DashboardLayout>
      <h1 className="font-display text-3xl font-extrabold text-forest">Profile</h1>
      <p className="mt-1 text-foreground/65">Manage your personal details, permissions, and account security.</p>
                ...
                <span
                  ...
                  className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider ${
                    role === "admin" ? "bg-amber-100 text-amber-800"
                      : role === "instructor" ? "bg-sky-100 text-sky-800"
      ...
      <div className="mt-6 max-w-4xl rounded-2xl bg-background border border-border p-6">
        ...
        <p className="text-xs text-foreground/55 mt-0.5">What your <span className="font-semibold capitalize">{primaryRole}</span> role lets you do.</p>
        <ul className="mt-4 grid sm:grid-cols-2 gap-2">
          {allowedPerms.map((p) => (
            <li key={p.action} className="flex items-start gap-2 rounded-lg border border-border bg-background p-3 text-sm">
              {p.allowed ? <Check className="h-4 w-4 mt-0.5 text-emerald-600 shrink-0" />
                         : <X className="h-4 w-4 mt-0.5 text-rose-600 shrink-0" />}
              <span className={p.allowed ? "text-forest" : "text-foreground/55 line-through"}>{p.action}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="mt-6 max-w-4xl rounded-2xl bg-background border border-border p-6">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-xs text-foreground/55 mt-0.5">Live pass/fail checks against the API to verify what you can actually do right now.</p>
          </div>
          <Button onClick={runAccessChecks} disabled={checking} variant="outline" className="rounded-full">
        ...
        {checks.length > 0 && (
          <ul className="mt-4 space-y-2">
            {checks.map((c) => (
              <li key={c.label} className="flex items-start justify-between gap-3 rounded-lg border border-border p-3 text-sm">
                <div className="flex items-start gap-2 min-w-0">
                  {c.pass ? <Check className="h-4 w-4 mt-0.5 text-emerald-600 shrink-0" />
                          : <X className="h-4 w-4 mt-0.5 text-rose-600 shrink-0" />}
                  <div className="min-w-0">
                    <p className="font-medium text-forest">{c.label}</p>
                    {c.detail && <p className="text-[11px] text-foreground/50 truncate">{c.detail}</p>}
                  </div>
                </div>
                <span className={`text-[11px] font-bold uppercase tracking-wider shrink-0 ${c.pass ? "text-emerald-700" : "text-rose-700"}`}>
                  {c.pass ? "Pass" : "Denied"}
                </span>
              </li>
            ))}
        ...
        {checks.length === 0 && !checking && (
          <p className="mt-4 text-xs text-foreground/55">Click <span className="font-semibold">Run checks</span> to probe your live API permissions.</p>
        )}
      </div>
      ...
      <div className="mt-6 max-w-4xl rounded-2xl bg-background border border-border p-6">
        ...
        <p className="text-xs text-foreground/55 mt-0.5">Submit a request and an admin will review it. You'll see the status below.</p>
        <form onSubmit={submitRoleRequest} className="mt-4 space-y-4">
          ...
          <Button type="submit" disabled={reqSubmitting} className="rounded-full bg-[#0A2E1A] text-mint hover:bg-[#0A2E1A]/90">
            {reqSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Send className="h-4 w-4 mr-1.5" /> Submit request</>}
          </Button>
        </form>

        {myRequests.length > 0 && (
          <div className="mt-6">
            <p className="text-[11px] uppercase tracking-[0.18em] font-bold text-secondary mb-2">Your requests</p>
            <ul className="space-y-2">
              {myRequests.map((r) => (
                <li key={r.id} className="rounded-lg border border-border p-3 text-sm flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    <p className="text-forest">
                      <span className="capitalize font-semibold">{r.from_role}</span> → <span className="capitalize font-semibold">{r.to_role}</span>
                    </p>
                    <p className="text-xs text-foreground/60 mt-0.5 line-clamp-2">{r.reason}</p>
                    {r.admin_notes && <p className="text-xs text-foreground/70 mt-1"><span className="font-semibold">Admin:</span> {r.admin_notes}</p>}
                    <p className="text-[11px] text-foreground/45 mt-1">{new Date(r.created_at).toLocaleString()}</p>
                  </div>
