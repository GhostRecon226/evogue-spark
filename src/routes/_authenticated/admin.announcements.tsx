...
function AdminAnnouncementsPage() {
  ...
  const load = async () => {
    ...
    const [annRes, cohortsRes, enrolRes] = await Promise.all([
      supabase.from("announcements").select("*, cohorts:cohort_id(name)").order("created_at", { ascending: false }),
      supabase.from("cohorts").select("id, name").order("name"),
      supabase.from("enrollments").select("cohort_id"),
    ]);
    const reach = new Map<string, number>();
    for (const e of enrolRes.data ?? []) if (e.cohort_id) reach.set(e.cohort_id, (reach.get(e.cohort_id) ?? 0) + 1);
    setCohorts(cohortsRes.data ?? []);
    setRows((annRes.data ?? []).map((a: any) => ({
      id: a.id, title: a.title, message: a.message, cohort_id: a.cohort_id,
      cohort_name: a.cohorts?.name ?? "—",
      created_at: a.created_at, reach: reach.get(a.cohort_id) ?? 0,
    })));
    setLoading(false);
  };
  ...
  const columns: Column<Announcement>[] = [
    ...
    { key: "reach", header: "Reach", accessor: (r) => r.reach,
      cell: (r) => <span className="font-bold text-forest">{r.reach} student{r.reach === 1 ? "" : "s"}</span> },
  ];
  ...
  return (
    <AdminGuard>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_2fr]">
          ...
          <div className="mt-4 grid gap-3">
            <div>
              ...
              <Select value={form.cohort_id} onValueChange={(v) => setForm({ ...form, cohort_id: v })}>
                <SelectTrigger className="rounded-lg"><SelectValue placeholder="Select cohort" /></SelectTrigger>
                <SelectContent>{cohorts.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div>
              <Label>Title</Label>
              <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Capstone deadline reminder" />
            </div>
            <div>
              <Label>Message</Label>
              <Textarea rows={6} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Write your announcement…" />
            </div>
            <Button onClick={post} disabled={posting} className="rounded-full bg-forest text-mint hover:bg-forest/90">
        ...
        <div>
          <h2 className="font-display text-lg font-extrabold text-forest mb-3">Past announcements</h2>
          {loading ? (
