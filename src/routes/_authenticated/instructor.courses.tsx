...
import { BookOpen, Loader2, Users, TrendingUp } from "lucide-react";
...
function InstructorCourses() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: courses }, { data: enrol }, { data: lessons }, { data: progress }] = await Promise.all([
        supabase.from("courses").select("id, title, slug, level").in("id", instructorCourseIds),
      ...
      setRows((courses ?? []).map((c) => {
        ...
        return { id: c.id, title: c.title, slug: c.slug, level: c.level, students: studentIds.length, completionRate: pct };
      }));
      setLoading(false);
    })();
