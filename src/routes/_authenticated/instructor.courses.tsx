...
import { BookOpen, Loader2, Users, TrendingUp, CalendarDays, CheckCircle2, XCircle } from "lucide-react";
...
function InstructorCourses() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: courses }, { data: enrol }, { data: lessons }, { data: progress }, { data: cohorts }] = await Promise.all([
        supabase.from("courses").select("id, title, slug, level, capstone_released").in("id", instructorCourseIds),
        ...
        supabase.from("cohorts").select("id, course_id, name, status, start_date").in("course_id", instructorCourseIds),
      ]);
      ...
      for (const p of progress ?? []) {
        ...
        completedByPair.set(`${p.student_id}:${p.course_id}`, (completedByPair.get(`${p.student_id}:${p.course_id}`) ?? 0) + 1);
      }
      ...
      setRows((courses ?? []).map((c) => {
        ...
        if (total > 0 && studentIds.length > 0) {
          const sum = studentIds.reduce((s, sid) => s + ((completedByPair.get(`${sid}:${c.id}`) ?? 0) / total) * 100, 0);
        ...
        return {
          ...
          students: studentIds.length, completionRate: pct,
        };
      }));
      setLoading(false);
    })();
            ...
            <div key={c.id} className="rounded-2xl border border-border bg-background p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-foreground/55">/{c.slug} · {c.level ?? "—"}</p>
                </div>
                <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-bold ${c.capstone_released ? "bg-secondary/15 text-forest" : "bg-mint-tint text-forest/60"}`}>
                  {c.capstone_released ? <CheckCircle2 className="h-3 w-3" /> : <XCircle className="h-3 w-3" />}
              ...
              <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-foreground/70">
                <span className="inline-flex items-center gap-1"><CalendarDays className="h-4 w-4 text-secondary" /> {c.activeCohort ?? "No active cohort"}</span>
                <span className="inline-flex items-center gap-1"><Users className="h-4 w-4 text-secondary" /> {c.students} students</span>
                <span className="inline-flex items-center gap-1"><TrendingUp className="h-4 w-4 text-secondary" /> {c.completionRate}% complete</span>
              </div>
              <div className="mt-3"><Progress value={c.completionRate} /></div>
            </div>
          ))}
        </div>
      )}
