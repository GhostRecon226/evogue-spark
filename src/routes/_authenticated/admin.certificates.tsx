...
function AdminCertificatesPage() {
  ...
  const [students, setStudents] = useState<
    { id: string; full_name: string | null; registration_number: string | null }[]
  >([]);
  ...
  const load = async () => {
    ...
    const [certsRes, studentsRes, coursesRes] = await Promise.all([
      supabase
        .from("certificates")
        .select(
          "*, profiles:student_id(full_name, registration_number), courses:course_id(title), cohorts:cohort_id(name)",
        )
  ...
  const issue = async () => {
    if (!form.student_id || !form.course_id) {
      toast.error("Student and course required");
      return;
    }
    setIssuing(true);
  ...
  return (
    <AdminGuard>
      ...
      <p className="mt-1 text-foreground/65">
        All certificates issued on completion of capstone projects.
      </p>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Manually issue a certificate to a student for a course.
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={!!revokeId} onOpenChange={(o) => !o && setRevokeId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              The student will lose access to this certificate. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
