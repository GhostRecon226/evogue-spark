...
function AdminCoursesPage() {
  ...
  const load = async () => {
    ...
    setRows(
      (coursesRes.data ?? []).map(
        (c) => ({ ...c, enrolled_count: counts.get(c.id) ?? 0 }) as CourseRow,
      ),
    );
    setLoading(false);
  };
  ...
  const startEdit = (c: CourseRow) => {
    ...
    setForm({
      ...
      capstone_brief: rest.capstone_brief ?? "",
  ...
  const columns: Column<CourseRow>[] = useMemo(
    () => [
      ...
      { key: "enrolled_count", header: "Enrolled", accessor: (r) => r.enrolled_count },
      {
        key: "is_published",
        header: "Published",
        accessor: (r) => (r.is_published ? 1 : 0),
        cell: (r) => (
          <Switch checked={r.is_published} onCheckedChange={(v) => togglePublished(r, v)} />
        ),
      },
    ],
    [],
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Fill in the details. Slug auto-generates from title if empty.
            </DialogDescription>
          </DialogHeader>
            ...
            <details className="rounded-xl border border-border p-3">
              ...
              <div className="mt-3 space-y-3">
                ...
                <div className="grid gap-1.5">
                  ...
                  <Textarea
                    rows={4}
                    value={form.capstone_brief ?? ""}
                    onChange={(e) => setForm({ ...form, capstone_brief: e.target.value })}
                  />
                </div>
                <div className="grid gap-1.5">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={save}
              disabled={saving}
              className="bg-forest text-mint hover:bg-forest/90"
            >
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently removes the course and cannot be undone. Existing enrollments will
              reference a missing course.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
