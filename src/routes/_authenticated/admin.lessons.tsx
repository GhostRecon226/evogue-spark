...
function AdminLessonsPage() {
  ...
  const load = async () => {
    ...
    const [lessonsRes, coursesRes, cohortsRes] = await Promise.all([
      supabase
        .from("lessons")
        .select("*, courses:course_id(title), cohorts:cohort_id(name)")
        .order("course_id")
        .order("lesson_number"),
  ...
  const columns: Column<Lesson>[] = [
    ...
    { key: "cohort_name", header: "Cohort", accessor: (r) => r.cohort_name ?? "—" },
    {
      key: "lesson_date",
      header: "Date",
      accessor: (r) => r.lesson_date,
      cell: (r) => (r.lesson_date ? new Date(r.lesson_date).toLocaleString() : "—"),
    },
    {
      key: "is_published",
      header: "Published",
      accessor: (r) => (r.is_published && r.cohort_id && r.lesson_date ? 1 : 0),
      cell: (r) => {
        const live = r.is_published && r.cohort_id && r.lesson_date;
        return live ? (
          <span className="inline-flex rounded-full bg-secondary/15 px-2.5 py-0.5 text-xs font-bold text-secondary">
            Live
          </span>
        ) : (
          <span className="inline-flex rounded-full bg-foreground/10 px-2.5 py-0.5 text-xs font-bold text-foreground/55">
            Draft
          </span>
        );
      },
  ...
  return (
    <AdminGuard>
      ...
      <p className="mt-1 text-foreground/65">
        Add live class links, recordings, and PDF resources per lesson.
      </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm({ ...form, course_id: v, cohort_id: "" })}
                >
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm({ ...form, cohort_id: v === "none" ? "" : v })}
                >
                  <SelectTrigger className="rounded-lg">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, lesson_number: parseInt(e.target.value || "1", 10) })
                  }
                />
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                <Input
                  value={form.zoom_live_link}
                  onChange={(e) => setForm({ ...form, zoom_live_link: e.target.value })}
              ...
              <div className="col-span-2">
                ...
                <Input
                  value={form.zoom_recording_link}
                  onChange={(e) => setForm({ ...form, zoom_recording_link: e.target.value })}
                  placeholder="https://…"
                />
              </div>
              <div className="col-span-2">
