...
const empty: Form = {
  ...
  status: "upcoming", capstone_released: false,
  capstone_brief_text: "", capstone_brief_url: "",
};
...
function AdminCohortsPage() {
  ...
  const load = async () => {
    ...
    const [cohortsRes, coursesRes, enrolRes] = await Promise.all([
      supabase.from("cohorts").select("*, courses:course_id(title)").order("created_at", { ascending: false }),
      supabase.from("courses").select("id, title").order("title"),
      supabase.from("enrollments").select("cohort_id"),
    ]);
    const counts = new Map<string, number>();
    for (const e of enrolRes.data ?? []) if (e.cohort_id) counts.set(e.cohort_id, (counts.get(e.cohort_id) ?? 0) + 1);
    setCourses(coursesRes.data ?? []);
    setRows((cohortsRes.data ?? []).map((c: any) => ({
      id: c.id, name: c.name, course_id: c.course_id,
      course_title: c.courses?.title ?? "—",
      start_date: c.start_date, end_date: c.end_date,
      status: c.status, capstone_released: c.capstone_released,
      capstone_brief_text: c.capstone_brief_text, capstone_brief_url: c.capstone_brief_url,
  ...
  const save = async () => {
    if (!form.name.trim() || !form.course_id) { toast.error("Name and course required"); return; }
    setSaving(true);
    const payload = {
      name: form.name.trim(),
  ...
  const toggleCapstone = async (c: Cohort) => {
    ...
    const { error } = await supabase.from("cohorts").update({ capstone_released: next }).eq("id", c.id);
  ...
  const columns: Column<Cohort>[] = [
    ...
    { key: "start_date", header: "Start", accessor: (r) => r.start_date,
      cell: (r) => r.start_date ? new Date(r.start_date).toLocaleDateString() : "—" },
    { key: "end_date", header: "End", accessor: (r) => r.end_date,
      cell: (r) => r.end_date ? new Date(r.end_date).toLocaleDateString() : "—" },
    { key: "status", header: "Status", accessor: (r) => r.status,
      cell: (r) => <span className="inline-block rounded-full bg-mint-tint px-2.5 py-1 text-xs font-bold text-forest capitalize">{r.status}</span> },
    { key: "enrolled_count", header: "Enrolled", accessor: (r) => r.enrolled_count },
    { key: "capstone_released", header: "Capstone", accessor: (r) => r.capstone_released ? 1 : 0,
      cell: (r) => (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Cohorts represent an intake of students for a specific course.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. January 2026" />
              </div>
              <div>
            ...
            <div>
              ...
              <Textarea rows={4} value={form.capstone_brief_text} onChange={(e) => setForm({ ...form, capstone_brief_text: e.target.value })} placeholder="Paste capstone instructions…" />
            </div>
            <div>
              <Label>Capstone brief (URL)</Label>
              <Input value={form.capstone_brief_url} onChange={(e) => setForm({ ...form, capstone_brief_url: e.target.value })} placeholder="https://…" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" className="rounded-full" onClick={() => setOpen(false)}>Cancel</Button>
            <Button className="rounded-full bg-forest text-mint hover:bg-forest/90" onClick={save} disabled={saving}>
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : (editing ? "Save changes" : "Create cohort")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
