...
type Lesson = {
  id: string; course_id: string; cohort_id: string | null; title: string; lesson_number: number;
  lesson_date: string | null; zoom_live_link: string | null; zoom_recording_link: string | null;
  pdf_url: string | null; is_published: boolean;
};
...
function UploadContent() {
  ...
  const cancelEdit = () => { setForm(emptyForm); setPdf(null); };

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.courseId) { toast.error("Select a course"); return; }
    if (!form.title.trim()) { toast.error("Lesson title is required"); return; }
    if (!Number.isFinite(form.lessonNumber) || form.lessonNumber < 1) { toast.error("Lesson number must be 1 or higher"); return; }
    ...
    <InstructorGuard>
      ...
      {courses.length === 0 ? (
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-5">
          <form onSubmit={submit} className="lg:col-span-3 space-y-5 rounded-2xl border border-border bg-background p-6">
            <div className="flex items-center justify-between">
              <h2 className="font-display font-bold text-forest">{form.id ? "Edit lesson" : "New lesson"}</h2>
              {form.id && (
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="grid gap-1.5">
                <Label>Course</Label>
                <Select value={form.courseId} onValueChange={(v) => setForm({ ...form, courseId: v, cohortId: "" })}>
              ...
              <div className="grid gap-1.5">
                <Label>Cohort</Label>
                <Select value={form.cohortId} onValueChange={(v) => setForm({ ...form, cohortId: v })} disabled={!form.courseId}>
                  <SelectTrigger><SelectValue placeholder={form.courseId ? "Select cohort…" : "Pick course first"} /></SelectTrigger>
                  <SelectContent>
                    {cohortsForCourse.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="grid gap-1.5">
                ...
                <Input type="number" min={1} value={form.lessonNumber}
                  onChange={(e) => setForm({ ...form, lessonNumber: parseInt(e.target.value, 10) || 1 })} />
              </div>
              <div className="grid gap-1.5">
                <Label>Lesson date & time</Label>
                <Input type="datetime-local" value={form.lessonDate}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="grid gap-1.5">
                ...
                <Input type="url" value={form.zoomLive} onChange={(e) => setForm({ ...form, zoomLive: e.target.value })} placeholder="https://zoom.us/j/…" />
              </div>
              <div className="grid gap-1.5">
                <Label>Zoom recording link</Label>
                <Input type="url" value={form.zoomRecording} onChange={(e) => setForm({ ...form, zoomRecording: e.target.value })} placeholder="https://zoom.us/rec/…" />
              </div>
            </div>
            <div className="grid gap-1.5">
                  ...
                  <div key={l.id} className="rounded-xl border border-border bg-background p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="font-semibold text-forest text-sm truncate">{l.lesson_number}. {l.title}</p>
                        <p className="text-xs text-foreground/55 truncate">{courseName}</p>
                      </div>
                      <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${l.is_published ? "bg-secondary/20 text-forest" : "bg-mint-tint text-foreground/60"}`}>
                        {l.is_published ? "Published" : "Draft"}
                      </span>
                    </div>
                    <Button variant="outline" size="sm" className="mt-3 rounded-full" onClick={() => startEdit(l)}>
