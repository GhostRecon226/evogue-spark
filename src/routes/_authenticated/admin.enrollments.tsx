...
function EnrollmentsPage() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from("enrollments")
      .select(
        "id, enrolled_at, payment_status, payment_reference, course_id, enrolled_by, profiles:student_id(full_name, email, registration_number), courses:course_id(title), cohorts:cohort_id(name)",
      )
      .order("enrolled_at", { ascending: false });
    const enrolledByIds = Array.from(
      new Set((data ?? []).map((r) => r.enrolled_by).filter(Boolean) as string[]),
    );
...
function NewEnrollmentDialog({
  ...
  createFn: ReturnType<typeof useServerFn<typeof adminCreateEnrollment>>;
  onCreated: () => void;
}) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setStudents(
        (s.data ?? []).map((p) => ({
          id: p.id,
          label: `${p.registration_number ?? ""} · ${p.full_name || p.email || ""}`,
        })),
      );
      setCourses(c.data ?? []);
      setCohorts(co.data ?? []);
  ...
  return (
    <DialogContent>
      <DialogHeader>
        <DialogTitle className="font-display text-forest">Create enrollment</DialogTitle>
      </DialogHeader>
      <form onSubmit={submit} className="grid gap-3 mt-2">
        ...
        <div>
          ...
          <Select value={form.cohort_id} onValueChange={(v) => setForm({ ...form, cohort_id: v })}>
            <SelectTrigger>
              <SelectValue
                placeholder={form.course_id ? "Select cohort…" : "Choose course first"}
              />
            </SelectTrigger>
            <SelectContent>
              {courseCohorts.map((c) => (
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(v: "paid" | "pending") => setForm({ ...form, payment_status: v })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
